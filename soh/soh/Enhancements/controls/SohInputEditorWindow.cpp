...
void SohInputEditorWindow::UpdateElement() {
    ...
    if (mInputEditorPopupOpen && ImGui::IsPopupOpen("", ImGuiPopupFlags_AnyPopupId)) {
        SohControlDeck()->BlockGameInput(INPUT_EDITOR_WINDOW_GAME_INPUT_BLOCK_ID);
        ...
        if (mGameInputBlockTimer != INT32_MAX) {
            ...
            if (mGameInputBlockTimer <= 0) {
                SohControlDeck()->UnblockGameInput(INPUT_EDITOR_WINDOW_GAME_INPUT_BLOCK_ID);
                mGameInputBlockTimer = INT32_MAX;
            }
        }

        if (SohWindow()->GetGui()->GamepadNavigationEnabled()) {
...
void SohInputEditorWindow::DrawButtonLineAddMappingButton(uint8_t port, N64ButtonMask bitmask) {
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX &&
            SohControlDeck()->GetControllerByPort(port)->GetButton(bitmask)->AddOrEditButtonMappingFromRawPress(bitmask,
                                                                                                                "")) {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
...
void SohInputEditorWindow::DrawButtonLineEditMappingButton(uint8_t port, N64ButtonMask bitmask, std::string id) {
    auto mapping = SohControlDeck()->GetControllerByPort(port)->GetButton(bitmask)->GetButtonMappingById(id);
    if (mapping == nullptr) {
        return;
    }
    ...
    if (sdlAxisDirectionToButtonMapping != nullptr) {
        ...
        if (ImGui::BeginPopup(popupId.c_str())) {
            ...
            auto globalSettings = SohControlDeck()->GetGlobalSDLDeviceSettings();
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeButtonMappingButton%s", ICON_FA_TIMES, id.c_str()).c_str(),
        ...
        SohControlDeck()->GetControllerByPort(port)->GetButton(bitmask)->ClearButtonMapping(id);
    };
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
    ImGui::PopStyleVar();
...
void SohInputEditorWindow::DrawStickDirectionLineAddMappingButton(uint8_t port, uint8_t stick,
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (stick == Ship::LEFT) {
            if (mMappingInputBlockTimer == INT32_MAX &&
                SohControlDeck()->GetControllerByPort(port)->GetLeftStick()->AddOrEditAxisDirectionMappingFromRawPress(
                    direction, "")) {
                mInputEditorPopupOpen = false;
                ImGui::CloseCurrentPopup();
            }
            ...
            if (mMappingInputBlockTimer == INT32_MAX &&
                SohControlDeck()->GetControllerByPort(port)->GetRightStick()->AddOrEditAxisDirectionMappingFromRawPress(
                    direction, "")) {
                ImGui::CloseCurrentPopup();
            }
        }
...
void SohInputEditorWindow::DrawStickDirectionLineEditMappingButton(uint8_t port, uint8_t stick,
    ...
    if (stick == Ship::LEFT) {
        mapping =
            SohControlDeck()->GetControllerByPort(port)->GetLeftStick()->GetAxisDirectionMappingById(direction, id);
    } else {
        mapping =
            SohControlDeck()->GetControllerByPort(port)->GetRightStick()->GetAxisDirectionMappingById(direction, id);
    }
    ...
    if (ImGui::Button(
            ...
            ImVec2(ImGui::CalcTextSize(ICON_FA_TIMES).x + SCALE_IMGUI_SIZE(10.0f), 0.0f))) {
        if (stick == Ship::LEFT) {
            SohControlDeck()->GetControllerByPort(port)->GetLeftStick()->ClearAxisDirectionMapping(direction, id);
        } else {
            SohControlDeck()->GetControllerByPort(port)->GetRightStick()->ClearAxisDirectionMapping(direction, id);
        }
    };
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
...
void SohInputEditorWindow::DrawStickSection(uint8_t port, uint8_t stick, int32_t id,
    ...
    if (stick == Ship::LEFT) {
        controllerStick = SohControlDeck()->GetControllerByPort(port)->GetLeftStick();
    } else {
        controllerStick = SohControlDeck()->GetControllerByPort(port)->GetRightStick();
    }
...
void SohInputEditorWindow::UpdateBitmaskToMappingIds(uint8_t port) {
    ...
    for (auto [bitmask, button] : SohControlDeck()->GetControllerByPort(port)->GetAllButtons()) {
...
void SohInputEditorWindow::UpdateStickDirectionToMappingIds(uint8_t port) {
    ...
    for (auto stick : { std::make_pair<uint8_t, std::shared_ptr<Ship::ControllerStick>>(
                            Ship::LEFT, SohControlDeck()->GetControllerByPort(port)->GetLeftStick()),
                        std::make_pair<uint8_t, std::shared_ptr<Ship::ControllerStick>>(
                            Ship::RIGHT, SohControlDeck()->GetControllerByPort(port)->GetRightStick()) }) {
...
void SohInputEditorWindow::DrawRemoveRumbleMappingButton(uint8_t port, std::string id) {
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeRumbleMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
        ...
        SohControlDeck()->GetControllerByPort(port)->GetRumble()->ClearRumbleMapping(id);
    }
    ImGui::PopStyleVar();
}
...
void SohInputEditorWindow::DrawAddRumbleMappingButton(uint8_t port) {
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX &&
            SohControlDeck()->GetControllerByPort(port)->GetRumble()->AddRumbleMappingFromRawPress()) {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
        ImGui::EndPopup();
...
void SohInputEditorWindow::DrawRumbleSection(uint8_t port) {
    for (auto [id, mapping] : SohControlDeck()->GetControllerByPort(port)->GetRumble()->GetAllRumbleMappings()) {
...
void SohInputEditorWindow::DrawRemoveLEDMappingButton(uint8_t port, std::string id) {
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeLEDMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
                      ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
        SohControlDeck()->GetControllerByPort(port)->GetLED()->ClearLEDMapping(id);
    }
    ImGui::PopStyleVar();
}
...
void SohInputEditorWindow::DrawAddLEDMappingButton(uint8_t port) {
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX &&
            SohControlDeck()->GetControllerByPort(port)->GetLED()->AddLEDMappingFromRawPress()) {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
        ImGui::EndPopup();
...
void SohInputEditorWindow::DrawLEDSection(uint8_t port) {
    for (auto [id, mapping] : SohControlDeck()->GetControllerByPort(port)->GetLED()->GetAllLEDMappings()) {
        ImGui::AlignTextToFramePadding();
        ImGui::SetNextItemOpen(true, ImGuiCond_Once);
...
void SohInputEditorWindow::DrawRemoveGyroMappingButton(uint8_t port, std::string id) {
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeGyroMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
                      ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
        SohControlDeck()->GetControllerByPort(port)->GetGyro()->ClearGyroMapping();
    }
    ImGui::PopStyleVar();
}
...
void SohInputEditorWindow::DrawAddGyroMappingButton(uint8_t port) {
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX &&
            SohControlDeck()->GetControllerByPort(port)->GetGyro()->SetGyroMappingFromRawPress()) {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
        ImGui::EndPopup();
...
void SohInputEditorWindow::DrawGyroSection(uint8_t port) {
    auto mapping = SohControlDeck()->GetControllerByPort(port)->GetGyro()->GetGyroMapping();
    if (mapping != nullptr) {
...
void SohInputEditorWindow::DrawDeviceToggles(uint8_t portIndex) {
    ...
    auto connectedDeviceManager = SohControlDeck()->GetConnectedPhysicalDeviceManager();
...
void SohInputEditorWindow::DrawClearAllButton(uint8_t portIndex) {
    ...
    if (ImGui::BeginPopupModal("Clear All##clearAllPopup", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::Button("Clear All")) {
            SohControlDeck()->GetControllerByPort(portIndex)->ClearAllMappings();
            ImGui::CloseCurrentPopup();
        }
        PopStyleButton();
        ImGui::EndPopup();
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (ImGui::BeginPopupModal("Set Defaults for Keyboard", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            if (ImGui::Button("Set defaults")) {
                SohControlDeck()->GetControllerByPort(portIndex)->ClearAllMappingsForDeviceType(
                    Ship::PhysicalDeviceType::Keyboard);
                SohControlDeck()->GetControllerByPort(portIndex)->AddDefaultMappings(
                    Ship::PhysicalDeviceType::Keyboard);
                shouldClose = true;
                ImGui::CloseCurrentPopup();
            }
