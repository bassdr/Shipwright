...
// Streamed tracks restart on another player to crossfade out. Each note decodes from its own
// decoder, so track how far each sample was decoded: the restarted note claims that position
// through Audio_NoteInitForLayer and continues where the cut note was instead of at the start.
...
void SOH_OpusStream_ArmContinue(struct OpusDecState* dec) {
    ...
    for (i = 0; i < 4; i++) {
        if (sOpusStreamContinue[i].source == dec->source) {
            sOpusStreamContinue[i].ttl = OPUS_STREAM_CONTINUE_TTL;
            return;
        }
    }
    sOpusStreamContinue[sOpusStreamContinueNext].source = dec->source;
